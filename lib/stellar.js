...
// Contract IDs
export const ESCROW_CONTRACT_ID = process.env.NEXT_PUBLIC_ESCROW_CONTRACT_ID || 'CADRPJIYYUUNLMJQYXJIMRUWEX752QII333QH3YP7EM5OFJO5GRW3VAH';
export const REPUTATION_CONTRACT_ID = process.env.NEXT_PUBLIC_REPUTATION_CONTRACT_ID || 'CD6K2TMLQBZZ7V3RT2KTAN2YZLVOUZ5OWQM4HZ2MC4DQXNOOTXT5UTO7';
...
export async function getCounter() {
    try {
        ...
        return response ? scValToNative(response.val) : 0;
    } catch (error) {
...
export async function getEscrow(id) {
    try {
        ...
        const native = scValToNative(response.val);
...
export async function getUserReputation(address) {
    try {
        ...
        return response ? scValToNative(response.val) : 100; // Default score
    } catch (error) {
        if (error.code === 404 || (error.response && error.response.status === 404)) {
...
export async function getUserAchievements(address) {
    try {
        ...
        const ids = response ? scValToNative(response.val) : [];
